make_exec_handler(mfc1) { GR_TV = cpu.fpr32i[ops->fs]; }
...
make_exec_handler(mfhc1) {
  GR_TV = cpu.fpr32i[ops->fs | 1];
}
make_exec_handler(mtc1) { cpu.fpr32i[ops->fs] = GR_TV; }
...
make_exec_handler(add_s) {
  float_status status = {};
  cpu.fpr32i[ops->fd] = float32_val(
      float32_add(make_float32(cpu.fpr32i[ops->fs]),
          make_float32(cpu.fpr32i[ops->ft]), &status));
}
make_exec_handler(add_d) {
  float_status status = {};
  cpu.fpr64i[ops->fd >> 1] = float64_val(
      float64_add(make_float64(cpu.fpr64i[ops->fs >> 1]),
          make_float64(cpu.fpr64i[ops->ft >> 1]), &status));
}
make_exec_handler(sub_s) {
  float_status status = {};
  cpu.fpr32i[ops->fd] = float32_val(
      float32_sub(make_float32(cpu.fpr32i[ops->fs]),
          make_float32(cpu.fpr32i[ops->ft]), &status));
}
make_exec_handler(sub_d) {
  float_status status = {};
  cpu.fpr64i[ops->fd >> 1] = float64_val(
      float64_sub(make_float64(cpu.fpr64i[ops->fs >> 1]),
          make_float64(cpu.fpr64i[ops->ft >> 1]), &status));
}
make_exec_handler(mul_s) {
  float_status status = {};
  cpu.fpr32i[ops->fd] = float32_val(
      float32_mul(make_float32(cpu.fpr32i[ops->fs]),
          make_float32(cpu.fpr32i[ops->ft]), &status));
}
make_exec_handler(mul_d) {
  float_status status = {};
  cpu.fpr64i[ops->fd >> 1] = float64_val(
      float64_mul(make_float64(cpu.fpr64i[ops->fs >> 1]),
          make_float64(cpu.fpr64i[ops->ft >> 1]), &status));
}
make_exec_handler(div_s) {
  float_status status = {};
  cpu.fpr32i[ops->fd] = float32_val(
      float32_div(make_float32(cpu.fpr32i[ops->fs]),
          make_float32(cpu.fpr32i[ops->ft]), &status));
}
make_exec_handler(div_d) {
  float_status status = {};
  cpu.fpr64i[ops->fd >> 1] = float64_val(
      float64_div(make_float64(cpu.fpr64i[ops->fs >> 1]),
          make_float64(cpu.fpr64i[ops->ft >> 1]), &status));
}

make_exec_handler(sqrt_s) {
  float_status status = {};
  cpu.fpr32i[ops->fd] = float32_val(float32_sqrt(
      make_float32(cpu.fpr32i[ops->fs]), &status));
}
make_exec_handler(sqrt_d) {
  float_status status = {};
  cpu.fpr64i[ops->fd >> 1] = float64_val(float64_sqrt(
      make_float64(cpu.fpr64i[ops->fs >> 1]), &status));
}

make_exec_handler(abs_s) {
  cpu.fpr32i[ops->fd] = float32_val(
      float32_abs(make_float32(cpu.fpr32i[ops->fs])));
}
make_exec_handler(abs_d) {
  cpu.fpr64i[ops->fd >> 1] = float64_val(
      float64_abs(make_float64(cpu.fpr64i[ops->fs >> 1])));
}

make_exec_handler(mov_s) {
  cpu.fpr32i[ops->fd] = cpu.fpr32i[ops->fs];
}
make_exec_handler(mov_d) {
  cpu.fpr64i[ops->fd >> 1] = cpu.fpr64i[ops->fs >> 1];
}

make_exec_handler(neg_s) {
  cpu.fpr32i[ops->fd] = cpu.fpr32i[ops->fs] ^ (1u << 31);
}
make_exec_handler(neg_d) {
  cpu.fpr64i[ops->fd >> 1] =
      cpu.fpr64i[ops->fs >> 1] ^ (1ull << 63);
}

make_exec_handler(trunc_w_s) {
  float_status status = {};
  cpu.fpr32i[ops->fd] = float32_val(float32_to_int32(
      make_float32(cpu.fpr32i[ops->fs]), &status));
}
make_exec_handler(trunc_w_d) {
  float_status status = {};
  cpu.fpr32i[ops->fd] = float32_val(float64_to_int32(
      make_float64(cpu.fpr64i[ops->fs >> 1]), &status));
}
...
make_exec_handler(movcf_d) {
  if (getFPCondCode(ops->cc2) == 0) {
    cpu.fpr64i[ops->fd64] = cpu.fpr32i[ops->fs64];
  }
}
...
make_exec_handler(movz_d) {
  if (GR_TV == 0) {
    cpu.fpr64i[ops->fd64] = cpu.fpr64i[ops->fs64];
  }
}
make_exec_handler(movn_s) {
  if (GR_TV != 0) {
    *(float *)&cpu.fpr32i[ops->fd] =
        *(float *)&cpu.fpr32i[ops->fs];
  }
}
make_exec_handler(movn_d) {
  if (GR_TV != 0) {
    *(double *)&cpu.fpr32i[ops->fd & ~1] =
        *(double *)&cpu.fpr32i[ops->fs & ~1];
  }
}
...
make_exec_handler(cvt_w_s) {
  cpu.fpr32i[ops->fd] =
      (int32_t) * (float *)&cpu.fpr32i[ops->fs];
}
...
make_exec_handler(cvt_w_d) {
  cpu.fpr32i[ops->fd] =
      (int32_t) * (double *)&cpu.fpr32i[ops->fs & ~1];
}
make_exec_handler(cvt_s_w) {
  *(float *)&cpu.fpr32i[ops->fd] =
      (int32_t)cpu.fpr32i[ops->fs];
}
...
make_exec_handler(c_eq_s) {
  setFPCondCode(
      ops->cc1, cpu.fpr32f[ops->fs] == cpu.fpr32f[ops->ft]);
}
make_exec_handler(c_eq_d) {
  setFPCondCode(ops->cc1,
      cpu.fpr64f[ops->fs64] == cpu.fpr64f[ops->ft64]);
}
...
make_exec_handler(swc1) {
  ...
  vaddr_write(waddr, 4, cpu.fpr32i[ops->ft]);
}
...
make_exec_handler(sdc1) {
  ...
  vaddr_write(waddr, 4, cpu.fpr32i[ops->ft & ~1]);
  vaddr_write(waddr + 4, 4, cpu.fpr32i[ops->ft | 1]);
}
