...
int tap_set_hwaddr(
  ...
  memcpy(ifr.ifr_hwaddr.sa_data, ether_addr, ETHER_ADDR_LEN);
...
int tap_set_up(int sockfd, const char *dev) {
  ...
  if (ioctl(sockfd, SIOCGIFFLAGS, (void *)&ifr) < 0) return -1;

  /* ifup tap0 # start up */
  ifr.ifr_flags |= IFF_UP;
  if (ioctl(sockfd, SIOCSIFFLAGS, (void *)&ifr) < 0) return -1;
  return 0;
}
...
int tap_set_attribute(const char *dev, const uint32_t ipaddr,
  ...
  if (tap_set_hwaddr(sockfd, dev, ether_addr) < 0) return -1;
  if (tap_set_ipaddr(sockfd, dev, ipaddr) < 0) return -1;
  if (tap_set_mtu(sockfd, dev, mtu) < 0) return -1;
  // if (tap_set_netmask(sockfd, dev, netmask) < 0) return -1;
