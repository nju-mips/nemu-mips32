...
#define INFO(_part_name, _jedec, _ext_jedec, _sector_size, _n_sectors, _flags) \
  .part_name = (_part_name), .jedec = (_jedec), .ext_jedec = (_ext_jedec),     \
...
static void flash_erase(Flash *s, int offset, FlashCMD cmd) {
  ...
  case ERASE_32K:
    ...
    break;
  case ERASE_SECTOR: len = s->pi->sector_size; break;
...
static void complete_collecting_data(Flash *s) {
  ...
  case DPP:
  case QPP:
  case PP: s->state = STATE_PAGE_PROGRAM; break;
  case READ:
  case FAST_READ:
  case DOR:
  case QOR:
  case DIOR:
  case QIOR: s->state = STATE_READ; break;
  case ERASE_4K:
  case ERASE_32K:
  case ERASE_SECTOR: flash_erase(s, s->cur_addr, s->cmd_in_progress); break;
  case WRSR:
...
static void decode_new_cmd(Flash *s, uint32_t value) {
  ...
  case DIOR:
    ...
    case JEDEC_WINBOND:
    case JEDEC_SPANSION: s->needed_bytes = 4; break;
    case JEDEC_NUMONYX:
    default: s->needed_bytes = 5;
    }
    s->pos = 0;
    s->len = 0;
    s->state = STATE_COLLECTING_DATA;
  ...
  case QIOR:
    switch ((s->pi->jedec >> 16) & 0xFF) {
    case JEDEC_WINBOND:
    case JEDEC_SPANSION: s->needed_bytes = 6; break;
    case JEDEC_NUMONYX:
    default: s->needed_bytes = 8;
    }
    s->pos = 0;
    s->len = 0;
    s->state = STATE_COLLECTING_DATA;
...
int m25p80_cs(Flash *s, bool select) {
  if (select) {
    s->len = 0;
    s->pos = 0;
    s->state = STATE_IDLE;
...
uint32_t m25p80_transfer8(Flash *s, uint32_t tx) {
  uint32_t r = 0;
