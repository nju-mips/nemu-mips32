...
#define ER_4K 1
#define ER_32K 2
/* set to allow the page program command to write 0s back
 * to 1. Useful for modelling EEPROM with SPI flash command
 * set
 */
#define EEPROM 0x100
...
typedef struct FlashPartInfo {
  ...
  uint8_t id_len;
  /* there is confusion between manufacturers as to what a
   * sector is. In this device model, a "sector" is the size
   * that is erased by the ERASE_SECTOR command (opcode
   * 0xd8).
   */
  ...
  uint16_t flags;
  /*
   * Big sized spi nor are often stacked devices, thus
   * sometime replace chip erase with die erase. This field
   * inform how many die is in the chip.
   */
  uint8_t die_cnt;
} FlashPartInfo;
...
#define INFO(_part_name, _jedec_id, _ext_id, _sector_size, \
    _n_sectors, _flags)                                    \
  ...
  .id_len =                                                \
      (!(_jedec_id) ? 0 : (3 + ((_ext_id) ? 2 : 0))),      \
  .sector_size = (_sector_size),                           \
  .n_sectors = (_n_sectors), .page_size = 256,             \
  .flags = (_flags), .die_cnt = 0

#define INFO6(_part_name, _jedec_id, _ext_id,  \
    _sector_size, _n_sectors, _flags)          \
  .part_name = _part_name,                     \
  .id =                                        \
      {                                        \
          ((_jedec_id) >> 16) & 0xff,          \
  ...
  .id_len = 6, .sector_size = (_sector_size),  \
  .n_sectors = (_n_sectors), .page_size = 256, \
  .flags = (_flags), .die_cnt = 0

#define INFO_STACKED(_part_name, _jedec_id, _ext_id,  \
    _sector_size, _n_sectors, _flags, _die_cnt)       \
  .part_name = _part_name,                            \
  .id =                                               \
      {                                               \
          ((_jedec_id) >> 16) & 0xff,                 \
...
static const FlashPartInfo known_devices[] = {
    ...
    /* Atmel EEPROMS - it is assumed, that don't care bit in
     * command is set to 0. Block protection is not
     * supported.
     */
    ...
    {INFO("mx66u51235f", 0xc2253a, 0, 64 << 10, 1024,
        ER_4K | ER_32K)},
    {INFO("mx66u1g45g", 0xc2253b, 0, 64 << 10, 2048,
        ER_4K | ER_32K)},
    {INFO("mx66l1g45g", 0xc2201b, 0, 64 << 10, 2048,
        ER_4K | ER_32K)},
    ...
    {INFO(
        "n25q512a11", 0x20bb20, 0, 64 << 10, 1024, ER_4K)},
    {INFO(
        "n25q512a13", 0x20ba20, 0, 64 << 10, 1024, ER_4K)},
    {INFO("n25q128", 0x20ba18, 0, 64 << 10, 256, 0)},
    {INFO("n25q256a", 0x20ba19, 0, 64 << 10, 512, ER_4K)},
    {INFO("n25q512a", 0x20ba20, 0, 64 << 10, 1024, ER_4K)},
    {INFO_STACKED("n25q00", 0x20ba21, 0x1000, 64 << 10,
        2048, ER_4K, 4)},
    {INFO_STACKED("n25q00a", 0x20bb21, 0x1000, 64 << 10,
        2048, ER_4K, 4)},
    {INFO_STACKED("mt25ql01g", 0x20ba21, 0x1040, 64 << 10,
        2048, ER_4K, 2)},
    {INFO_STACKED("mt25qu01g", 0x20bb21, 0x1040, 64 << 10,
        2048, ER_4K, 2)},

    /* Spansion -- single (large) sector size only, at least
     * for the chips listed here (without boot sectors).
     */
    {INFO("s25sl032p", 0x010215, 0x4d00, 64 << 10, 64,
        ER_4K)},
    {INFO("s25sl064p", 0x010216, 0x4d00, 64 << 10, 128,
        ER_4K)},
    {INFO(
        "s25fl256s0", 0x010219, 0x4d00, 256 << 10, 128, 0)},
    {INFO(
        "s25fl256s1", 0x010219, 0x4d01, 64 << 10, 512, 0)},
    {INFO6("s25fl512s", 0x010220, 0x4d0080, 256 << 10, 256,
        0)},
    {INFO6("s70fl01gs", 0x010221, 0x4d0080, 256 << 10, 512,
        0)},
    {INFO(
        "s25sl12800", 0x012018, 0x0300, 256 << 10, 64, 0)},
    {INFO(
        "s25sl12801", 0x012018, 0x0301, 64 << 10, 256, 0)},
    {INFO(
        "s25fl129p0", 0x012018, 0x4d00, 256 << 10, 64, 0)},
    {INFO(
        "s25fl129p1", 0x012018, 0x4d01, 64 << 10, 256, 0)},
    ...
    {INFO("s25fl016k", 0xef4015, 0, 64 << 10, 32,
        ER_4K | ER_32K)},
    {INFO("s25fl064k", 0xef4017, 0, 64 << 10, 128,
        ER_4K | ER_32K)},

    /* Spansion --  boot sectors support  */
    {INFO6("s25fs512s", 0x010220, 0x4d0081, 256 << 10, 256,
        0)},
    {INFO6("s70fs01gs", 0x010221, 0x4d0081, 256 << 10, 512,
        0)},

    /* SST -- large erase sizes are "overlays", "sectors"
       are 4<< 10 */
    ...
    /* ST Microelectronics -- newer production may have
       feature updates */
...
static inline uint32_t deposit32(uint32_t value, int start,
    int length, uint32_t fieldval) {
  uint32_t mask;
  assert(start >= 0 && length > 0 && length <= 32 - start);
  mask = (~0U >> (32 - length)) << start;
  return (value & ~mask) | ((fieldval << start) & mask);
}
static inline uint32_t extract32(
    uint32_t value, int start, int length) {
  assert(start >= 0 && length > 0 && length <= 32 - start);
...
static void flash_erase(
    Flash *s, int offset, FlashCMD cmd) {
  uint32_t len;
  ...
  case DIE_ERASE:
    if (s->pi->die_cnt) {
      ...
      DB_PRINT_L(
          "M25P80: die erase is not supported by device\n");
      return;
    }
    break;
  default: abort();
...
static inline void flash_sync_dirty(
    Flash *s, int64_t newpage) {
...
static inline void flash_write8(
    Flash *s, uint32_t addr, uint8_t data) {
  uint32_t page = addr / s->pi->page_size;
  uint8_t prev = s->storage[s->cur_addr];

  if (!s->write_enable) {
    DB_PRINT_L("M25P80: write with write protect!\n");
  }

  if ((prev ^ data) & data) {
    DB_PRINT_L("programming zero to one! addr=%" PRIx32
               "  %" PRIx8 " -> %" PRIx8 "\n",
        addr, prev, data);
  }
...
static void reset_memory(Flash *s) {
  ...
  case MAN_NUMONYX:
    ...
    s->volatile_cfg |= deposit32(s->volatile_cfg,
        VCFG_DUMMY_CLK_POS, CFG_DUMMY_CLK_LEN,
        extract32(s->nonvolatile_cfg, NVCFG_DUMMY_CLK_POS,
            CFG_DUMMY_CLK_LEN));
  ...
  case MAN_SPANSION:
    ...
    s->quad_enable = extract32(s->spansion_cr1v,
        SPANSION_QUAD_CFG_POS, SPANSION_QUAD_CFG_LEN);
    s->four_bytes_address_mode = extract32(s->spansion_cr2v,
        SPANSION_ADDR_LEN_POS, SPANSION_ADDR_LEN_LEN);
    break;
  default: break;
  }
...
static void decode_fast_read_cmd(Flash *s) {
  ...
  case MAN_SPANSION:
    s->needed_bytes += extract32(s->spansion_cr2v,
        SPANSION_DUMMY_CLK_POS, SPANSION_DUMMY_CLK_LEN);
    break;
  default: break;
  }
  s->pos = 0;
...
static void decode_new_cmd(Flash *s, uint32_t value) {
  ...
  default:
    DB_PRINT_L("M25P80: Unknown cmd %x\n", value);
    break;
  }
}
...
static uint32_t m25p80_transfer8(Flash *s, uint32_t tx) {
  ...
  case STATE_PAGE_PROGRAM:
    DB_PRINT_L("page program cur_addr=%#" PRIx32
               " data=%" PRIx8 "\n",
...
static void flash_init(Flash *s) {
  /* FIXME:
