...
uint32_t mac_read(paddr_t addr, int len) {
  ...
  default:
    CPUAssert(false, "mac: address(0x%08x) is not readable",
              addr);
    break;
  }
  return 0;
...
void mac_write(paddr_t addr, int len, uint32_t data) {
  ...
  case TX_PING_TSR:
    ...
    if (regs.tx_ping_tsr & XEL_TSR_PROGRAM_MASK) {
      memcpy(&(ping_mac.addr[0]), &regs.tx_ping,
             regs.tx_ping_tplr);
  ...
  case TX_PONG_TSR:
    ...
    if (regs.tx_pong_tsr & XEL_TSR_PROG_MAC_ADDR) {
      memcpy(&(pong_mac.addr[0]), &regs.tx_pong,
             regs.tx_pong_tplr);
  ...
  default:
    CPUAssert(false, "mac: address(0x%08x) is not writable",
              addr);
    break;
  }
}
