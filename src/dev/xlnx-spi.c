...
static void spi_flush_txfifo() {
  while (!fifo_is_empty(spi_tx_fifo)) {
    ...
    if (!(xlnx_spi_regs[R_SPISSR] & 1))
      rx = m25p80_transfer8(&flash, tx);
...
static void xlnx_spi_write(
    uint32_t addr, int len, uint32_t data) {
  addr >>= 2;
  switch (addr) {
  case R_SRR:
...
static void xlnx_spi_set_blkio_file(const char *file) {
  flash_set_blkio_file(&flash, file);
}
